...
export const createProject: Handler<APIGatewayEvent, APIGatewayProxyResult> = async (event) => {
  try {
...
export const updateProject: Handler<APIGatewayEvent, APIGatewayProxyResult> = async (event) => {
  try {
    const projectId = parseInt(event.pathParameters!.id as string);
    const data: Partial<Project> = JSON.parse(event.body!);
    const updatedProject = await db.update(tbl_projects).set(data).where(eq(tbl_projects.id, projectId)).execute();
...
export const listProjects: Handler<APIGatewayEvent, APIGatewayProxyResult> = async () => {
  try {
    const projects: ProjectList[] = await db.select({
      id: tbl_projects.id,
      name: tbl_projects.name,
      createdAt: tbl_projects.createdAt,
      updatedAt: tbl_projects.updatedAt,
    }).from(tbl_projects).execute();
